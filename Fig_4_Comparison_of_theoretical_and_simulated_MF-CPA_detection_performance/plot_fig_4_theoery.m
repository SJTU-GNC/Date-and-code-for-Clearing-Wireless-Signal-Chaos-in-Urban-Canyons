...
[X, Y] = meshgrid(0:1:250, 1:1:20);

plot_func = @(Z, name) plot_data(Z, X, Y, custom_map, name);

plot_func(performance_GPS_theoery_tau_alp{1}, 'GPS');
plot_func(performance_BDS_theoery_tau_alp{1}, 'BDS');
plot_func(performance_Gal_theoery_tau_alp{1}, 'Galileo');
plot_func(performance_GLO_theoery_tau_alp{1}, 'GLONASS');
...
function plot_data(Z, X, Y, cmap, name)
    ...
    set(gca, 'FontName', 'Helvetica', 'FontSize', 25, ...
        ...
        'YTick', [9.5, 11.5, 13.5, 15.5, 17.5, 19.5], ...
